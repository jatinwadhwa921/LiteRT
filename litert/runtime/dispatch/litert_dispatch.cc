...
LiteRtStatus RegisterTensorBufferHandlers(LiteRtEnvironment env) {
  ...
  for (size_t i = 0;
       ...
       i < LITERT_CUSTOM_BUFFER_HANDLERS_DEF_MAX_SUPPORTED_BUFFER_TYPES; ++i) {
...
LiteRtStatus LiteRtDispatchInitialize(
  ...
  if (IsTheApiInitialized) {
    return RegisterTensorBufferHandlers(env);
  }
  LiteRtEnvironmentOptions env_options;
  ...
  LITERT_RETURN_IF_ERROR(RegisterTensorBufferHandlers(env));
